...
import {
  ...
  Star, ExternalLink, ArrowRight, DollarSign, Target, Zap,
  Check, Award, Clock
} from "lucide-react";
...
const EMAIL_TEMPLATES = [
  { id: 1, name: "Presentación",    icon: "📧", subject: "Presentación de nuestros servicios – {empresa}", body: `Estimado/a {nombre},\n\nEspero que se encuentre muy bien. Me permito contactarle para presentarle nuestros servicios de consultoría, los cuales han ayudado a empresas como la suya a optimizar sus procesos y aumentar su rentabilidad.\n\nNos especializamos en soluciones a medida que se adaptan a las necesidades específicas de cada cliente.\n\n¿Tendría disponibilidad para una breve llamada esta semana?\n\nQuedo a sus órdenes,\n{vendedor}` },
  { id: 2, name: "Seguimiento",     icon: "🔁", subject: "Seguimiento – Propuesta para {empresa}", body: `Estimado/a {nombre},\n\nMe pongo en contacto para hacer seguimiento a la propuesta que le compartimos hace unos días.\n\n¿Ha tenido oportunidad de revisarla? Estoy disponible para resolver cualquier duda o ajustar la propuesta.\n\nQuedo en espera de sus comentarios.\n\nSaludos cordiales,\n{vendedor}` },
  { id: 3, name: "Propuesta",       icon: "📋", subject: "Propuesta personalizada para {empresa}", body: `Estimado/a {nombre},\n\nAdjunto encontrará nuestra propuesta personalizada para {empresa}.\n\nEn ella encontrará:\n• Descripción del servicio propuesto\n• Cronograma de trabajo\n• Inversión requerida\n• Garantías y condiciones\n\n¿Cuándo le vendría bien agendar una reunión para presentársela?\n\nSaludos,\n{vendedor}` },
  { id: 4, name: "Reactivación",    icon: "🔥", subject: "¡Nos gustaría retomar el contacto!", body: `Estimado/a {nombre},\n\nHace algún tiempo tuvimos el gusto de platicar sobre cómo podíamos apoyarle.\n\nHemos lanzado nuevos servicios y promociones especiales que creo podrían interesarle.\n\n¿Le gustaría que le compartiera las novedades?\n\nSaludos,\n{vendedor}` },
];

const SAMPLE_CLIENTS = [
  { id: "s1", name: "Ana García",      phone: "8112345678", email: "ana@techcorp.mx",    company: "Tech Corp MX",   stage: "Ganado",      value: 45000, notes: "Cliente recurrente, muy satisfecha.", created: "2026-02-10T10:00:00Z" },
  { id: "s2", name: "Carlos López",    phone: "8189876543", email: "carlos@negocios.mx", company: "Negocios MX",    stage: "Propuesta",   value: 28000, notes: "Esperando aprobación interna.",        created: "2026-02-20T09:00:00Z" },
  { id: "s3", name: "María Rodríguez", phone: "5544556677", email: "maria@startup.io",   company: "Startup IO",     stage: "Negociación", value: 62000, notes: "Interesada en paquete completo.",      created: "2026-02-25T11:00:00Z" },
  { id: "s4", name: "Roberto Herrera", phone: "6641234567", email: "r@empresa.com",       company: "Empresa SA",     stage: "Contactado",  value: 15000, notes: "Llamada inicial realizada.",           created: "2026-03-01T14:00:00Z" },
  { id: "s5", name: "Sofía Martínez",  phone: "3339876543", email: "sofia@grupomtz.com", company: "Grupo Martínez", stage: "Lead",        value: 20000, notes: "Referida por Ana García.",            created: "2026-03-02T16:00:00Z" },
];
...
function Avatar({ name, size = 8 }) {
  ...
  return (
    <div className={`${sz} rounded-full ${colors[i]} flex items-center justify-center text-white font-bold text-xs flex-shrink-0`}>
      {initials}
    </div>
  );
}
  ...
  return (
    <Modal title={client ? "Editar Cliente" : "Agregar Cliente"} onClose={onClose} wide>
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            {STAGES.map(s => (
              <button key={s} type="button" onClick={()=>set("stage",s)} className={`py-2 px-3 rounded-xl text-xs font-semibold border-2 transition-all ${form.stage===s ? "border-indigo-500 bg-indigo-50 text-indigo-700" : "border-gray-200 text-gray-600 hover:border-gray-300"}`}>
                {STAGE_META[s].label}
              </button>
            ))}
          </div>
      ...
      <div className="flex gap-3 mt-6">
        <button type="button" onClick={onClose} className="flex-1 border border-gray-200 text-gray-600 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-50 transition-colors">Cancelar</button>
        <button type="button" onClick={handleSave} className="flex-1 bg-indigo-600 text-white py-2.5 rounded-xl text-sm font-medium hover:bg-indigo-700 transition-colors">
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        {[
          ...
          { label:"Tasa de cierre",  value:`${rate}%`,                  icon:Award,      color:"bg-emerald-500", sub:"clientes ganados"      },
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <h3 className="font-bold text-gray-900 mb-5 flex items-center gap-2"><BarChart3 size={18} className="text-indigo-500"/>Pipeline por etapa</h3>
      ...
      <div className="mt-6 grid grid-cols-3 gap-4">
        {[
          { label:"Enviar WhatsApp", sub:"Contacta clientes rápido", icon:MessageCircle, view:"whatsapp", color:"bg-green-50 text-green-700 border-green-200" },
          { label:"Enviar Email",    sub:"Campañas de correo",        icon:Mail,          view:"email",    color:"bg-blue-50 text-blue-700 border-blue-200"   },
          { label:"Ver Pipeline",    sub:"Gestiona oportunidades",    icon:TrendingUp,    view:"pipeline", color:"bg-indigo-50 text-indigo-700 border-indigo-200" },
          ...
          <button key={v} onClick={()=>setView(v)} className={`flex items-center gap-4 p-5 rounded-2xl border-2 ${color} hover:shadow-md transition-all text-left`}>
            <Icon size={24}/><div><div className="font-semibold text-sm">{label}</div><div className="text-xs opacity-70 mt-0.5">{sub}</div></div>
...
function ClientsView({ clients, onAdd, onEdit, onDelete, onMove }) {
  ...
  const filtered = clients.filter(c => {
    ...
    const match = c.name.toLowerCase().includes(q) || (c.email||"").toLowerCase().includes(q) || (c.company||"").toLowerCase().includes(q) || (c.phone||"").includes(q);
    return match && (filter === "Todos" || c.stage === filter);
  });
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">{clients.length} contactos registrados</p>
        </div>
        <button onClick={onAdd} className="flex items-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-indigo-700 transition-colors shadow-sm">
      ...
      <div className="flex gap-4 mb-6 flex-wrap">
        <div className="flex-1 min-w-48 relative">
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar por nombre, empresa, email..." className="w-full pl-11 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white"/>
        </div>
        <div className="flex gap-2 flex-wrap">
          {["Todos", ...STAGES].map(s => (
            <button key={s} onClick={()=>setFilter(s)} className={`px-4 py-3 rounded-xl text-xs font-semibold transition-all border ${filter===s ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-gray-600 border-gray-200 hover:border-gray-300"}`}>{s}</button>
          ))}
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map(c => (
                <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-4 text-sm text-gray-600 whitespace-nowrap">{c.company}</td>
                  <td className="px-5 py-4">
                    <div className="flex gap-2">
                      {c.phone && (
                        <a href={`https://wa.me/52${c.phone}`} target="_blank" rel="noreferrer" className="w-8 h-8 rounded-full bg-green-50 text-green-600 flex items-center justify-center hover:bg-green-100 transition-colors">
                          <MessageCircle size={14}/>
                        </a>
                      )}
                      {c.email && (
                        <a href={`mailto:${c.email}`} className="w-8 h-8 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-blue-100 transition-colors">
                          <Mail size={14}/>
                        </a>
                      )}
                    </div>
                  ...
                  <td className="px-5 py-4">
                    <select value={c.stage} onChange={e=>onMove(c.id,e.target.value)} className="text-xs font-semibold rounded-lg px-2 py-1 border-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-500" style={{backgroundColor: STAGE_META[c.stage]?.bg, color: STAGE_META[c.stage]?.color}}>
                      {STAGES.map(s=><option key={s} value={s}>{s}</option>)}
                    </select>
                  </td>
                  <td className="px-5 py-4 text-sm font-bold text-gray-700 whitespace-nowrap">${(c.value||0).toLocaleString()}</td>
                  <td className="px-5 py-4 text-xs text-gray-500 max-w-xs truncate">{c.notes || "—"}</td>
                  <td className="px-5 py-4">
                    <div className="flex gap-2">
                      <button onClick={()=>onEdit(c)} className="w-8 h-8 rounded-full hover:bg-indigo-50 text-gray-400 hover:text-indigo-600 flex items-center justify-center transition-colors"><Edit2 size={14}/></button>
                      <button onClick={()=>onDelete(c.id)} className="w-8 h-8 rounded-full hover:bg-red-50 text-gray-400 hover:text-red-500 flex items-center justify-center transition-colors"><Trash2 size={14}/></button>
                    </div>
                  </td>
                </tr>
              ))}
...
function PipelineView({ clients, onMove, onEdit }) {
  const active = STAGES.filter(s=>s!=="Perdido");
  return (
    <div className="p-8 flex flex-col" style={{height:"calc(100vh - 0px)"}}>
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">Gestiona el avance de cada oportunidad</p>
      </div>
            ...
            <div key={stage} className="flex-shrink-0 w-64 flex flex-col">
              <div className="rounded-2xl p-4 mb-3" style={{backgroundColor: meta.bg}}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-bold uppercase tracking-wide" style={{color: meta.color}}>{stage}</span>
                  <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-white" style={{color: meta.color}}>{sc.length}</span>
                </div>
                <div className="text-sm font-bold text-gray-700">${total.toLocaleString()} MXN</div>
              </div>
              <div className="flex-1 space-y-3 overflow-y-auto">
                {sc.map(c => (
                  <div key={c.id} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                    ...
                    <div className="text-base font-bold text-gray-800 mb-2">${(c.value||0).toLocaleString()}</div>
                    {c.notes && <p className="text-xs text-gray-500 mb-3 line-clamp-2">{c.notes}</p>}
                    <div className="flex gap-1 flex-wrap">
                      {STAGES.filter(s=>s!==stage).slice(0,3).map(s=>(
                        <button key={s} onClick={()=>onMove(c.id,s)} className="text-xs px-2 py-1 rounded-lg border border-gray-200 text-gray-500 hover:border-indigo-300 hover:text-indigo-600 transition-colors">→ {s}</button>
                      ))}
                    </div>
                    <button onClick={()=>onEdit(c)} className="mt-2 w-full text-xs text-indigo-600 font-medium py-1 flex items-center justify-center gap-1">
...
function WhatsAppView({ clients }) {
  ...
  const openWA = (c) => {
    ...
    window.open(`https://wa.me/52${phone}?text=${encodeURIComponent(buildMsg(c))}`, "_blank");
    ...
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
          <span className="w-10 h-10 bg-green-500 rounded-xl flex items-center justify-center"><MessageCircle size={22} className="text-white"/></span>
          Mensajes WhatsApp
        </h1>
        <p className="text-gray-500 mt-1">Selecciona clientes y envía mensajes personalizados</p>
      </div>
        ...
        <div className="col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col" style={{maxHeight:"600px"}}>
          <div className="p-4 border-b border-gray-100">
            <div className="relative mb-3">
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar..." className="w-full pl-9 pr-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500"/>
            </div>
            <div className="flex gap-2">
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
            {filtered.map(c => (
              <div key={c.id} onClick={()=>toggle(c.id)} className={`flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-gray-50 ${selected.includes(c.id)?"bg-green-50":""}`}>
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${selected.includes(c.id)?"border-green-500 bg-green-500":"border-gray-300"}`}>
                  {selected.includes(c.id) && <Check size={11} className="text-white"/>}
                </div>
          ...
          <div className="p-4 border-t border-gray-100 bg-gray-50 rounded-b-2xl text-xs text-gray-500">{selected.length} seleccionados</div>
        </div>
        <div className="col-span-3 space-y-4">
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
            <label className="block text-sm font-semibold text-gray-700 mb-2">Tu nombre (vendedor)</label>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
              {WA_TEMPLATES.map(t => (
                <button key={t.id} onClick={()=>{ setTemplate(t); setCustomMsg(t.text); }} className={`p-3 rounded-xl border-2 text-xs font-medium text-left ${template.id===t.id?"border-green-500 bg-green-50":"border-gray-200 hover:border-gray-300"}`}>
          ...
          {selected.length > 0 && (
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
              <div className="text-sm font-semibold text-gray-700 mb-3">Enviar individualmente</div>
                ...
                {clients.filter(c=>selected.includes(c.id)).map(c=>(
                  <div key={c.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-400 truncate">{buildMsg(c).substring(0,50)}...</div>
                    </div>
                    <button onClick={()=>openWA(c)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${sent[c.id]?"bg-green-100 text-green-700":"bg-green-500 text-white hover:bg-green-600"}`}>
                      {sent[c.id]?"✓ Listo":"Enviar"}
                    </button>
                  </div>
                ))}
              </div>
          ...
          {selected.length === 0 && (
            <div className="text-center py-10 text-gray-400 bg-white rounded-2xl border border-dashed border-gray-200">
              ...
              <p className="text-sm">Selecciona clientes de la lista</p>
            </div>
          )}
        </div>
      </div>
...
function EmailView({ clients }) {
  ...
  const fill = (c) => ({
    ...
    b: body.replace(/\{nombre\}/gi,c.name.split(" ")[0]).replace(/\{empresa\}/gi,c.company||"").replace(/\{vendedor\}/gi,vendedor),
  });
  const openEmail = (c) => {
    const { s, b } = fill(c);
    window.open(`mailto:${c.email}?subject=${encodeURIComponent(s)}&body=${encodeURIComponent(b)}`);
    setSent(st=>({...st,[c.id]:true}));
  };
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
          <span className="w-10 h-10 bg-blue-500 rounded-xl flex items-center justify-center"><Mail size={22} className="text-white"/></span>
          Campañas de Email
        </h1>
        <p className="text-gray-500 mt-1">Envía correos personalizados a tus clientes</p>
      </div>
      <div className="grid grid-cols-5 gap-6">
        <div className="col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col" style={{maxHeight:"600px"}}>
          <div className="p-4 border-b border-gray-100">
            <div className="relative mb-3">
              <Search size={15} className="absolute left-3 top-3 text-gray-400"/>
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar..." className="w-full pl-9 pr-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
            <div className="flex gap-2">
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
            {filtered.map(c => (
              <div key={c.id} onClick={()=>c.email && toggle(c.id)} className={`flex items-center gap-3 px-4 py-3 ${c.email?"cursor-pointer hover:bg-gray-50":"opacity-40 cursor-not-allowed"} ${selected.includes(c.id)?"bg-blue-50":""}`}>
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${selected.includes(c.id)?"border-blue-500 bg-blue-500":"border-gray-300"}`}>
                  {selected.includes(c.id) && <Check size={11} className="text-white"/>}
                </div>
                <Avatar name={c.name} size={8}/>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold text-gray-800 truncate">{c.name}</div>
                  <div className="text-xs text-gray-400 truncate">{c.email || "Sin email"}</div>
                </div>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
            ...
            <div className="grid grid-cols-4 gap-2 mb-4">
              {EMAIL_TEMPLATES.map(t => (
                <button key={t.id} onClick={()=>{ setTemplate(t); setSubject(t.subject); setBody(t.body); }} className={`p-2.5 rounded-xl border-2 text-xs font-medium ${template.id===t.id?"border-blue-500 bg-blue-50":"border-gray-200 hover:border-gray-300"}`}>
                  <div className="text-lg mb-1">{t.icon}</div>{t.name}
                </button>
              ))}
            </div>
            <div className="space-y-3">
              <div>
                ...
                <input value={subject} onChange={e=>setSubject(e.target.value)} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Cuerpo del correo</label>
                <textarea value={body} onChange={e=>setBody(e.target.value)} rows={8} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none font-mono"/>
              </div>
            </div>
          </div>
          {selected.length > 0 && (
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
              ...
              <p className="text-xs text-gray-400 mt-3 flex items-center gap-1"><ExternalLink size={11}/>Se abrirá tu cliente de correo predeterminado con el mensaje listo.</p>
            </div>
          )}
        </div>
      </div>
...
export default function SalesFlow() {
  ...
  useEffect(()=>{
    try {
      const saved = localStorage.getItem("salesflow-clients");
      setClients(saved ? JSON.parse(saved) : SAMPLE_CLIENTS);
    } catch(e) { setClients(SAMPLE_CLIENTS); }
    setLoading(false);
  },[]);

  const save = (updated) => {
    setClients(updated);
    try { localStorage.setItem("salesflow-clients", JSON.stringify(updated)); } catch(e){}
  };

  const addClient    = (c) => { save([...clients, {...c, id: Date.now().toString(), created: new Date().toISOString()}]); setShowAdd(false); };
  const updateClient = (c) => { save(clients.map(x=>x.id===c.id?c:x)); setEditing(null); };
  const deleteClient = (id) => { if(window.confirm("¿Eliminar este cliente?")) save(clients.filter(c=>c.id!==id)); };
  const moveStage    = (id,s) => save(clients.map(c=>c.id===id?{...c,stage:s}:c));

  if(loading) return (
    <div className="flex items-center justify-center h-screen bg-gray-50">
      <div className="text-center">
        <div className="w-12 h-12 border-4 border-indigo-200 border-t-indigo-600 rounded-full animate-spin mx-auto mb-4"/>
        <p className="text-gray-500 text-sm">Cargando SalesFlow...</p>
      </div>
    </div>
  );
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden" style={{fontFamily:"'Segoe UI', system-ui, sans-serif"}}>
      <div className="w-60 flex-shrink-0 flex flex-col" style={{background:"linear-gradient(180deg,#1e1b4b 0%,#312e81 100%)"}}>
        <div className="p-6 border-b border-white border-opacity-10">
          ...
          <div className="text-indigo-300 text-xs">Sistema de Ventas Pro</div>
        </div>
        ...
        <div className="p-4 border-t border-white border-opacity-10">
          ...
          <div className="text-indigo-400 text-xs mt-3 text-center">{clients.length} clientes · v1.0</div>
        </div>
      </div>
